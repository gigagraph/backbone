    ...
    opts = {
      ...
      color_overrides = {
        latte = {
          ...
          -- The name is: Industrial Summer Paper Sunset.
          crust = "#e0e0e0",
          mantle = "#f0f0f0",
          base = "#ffffff",
          surface0 = "#dddddd",
        ...
        mocha = {
          ...
          -- It needs a custom name too - Catppuccin Dark Cocoa.
          crust = "#050505",
          mantle = "#101010",
          base = "#202020",
          surface0 = "#383838",
...
---- nvim-nio
local nvim_dap_ui_lazy_spec = bpu:declare_lazy_spec(
  "config.infra.plugins.nvim-nio",
  {
    config = function(lazy_plugin, opts) end
  }
)

---- nvim-dap
local nvim_dap_lazy_spec = bpu:declare_lazy_spec(
  "config.infra.plugins.nvim-dap",
  {
    dependencies = {
      "nvim-dap-virtual-text",
    },
    config = function(lazy_plugin, opts)
      local dap = require(lazy_plugin.name)

      -- Define keybindings for debugging
...
---- nvim-dap-ui
local nvim_dap_ui_lazy_spec = bpu:declare_lazy_spec(
  "config.infra.plugins.nvim-dap-ui",
  {
    dependencies = {
      "dap",
      "nvim-dap-virtual-text",
...
---- nvim-dap-virtual-text
local nvim_dap_virtual_text_lazy_spec = bpu:declare_lazy_spec(
  "config.infra.plugins.nvim-dap-virtual-text",
  {
    opts = {
      ...
      display_callback = function(variable, buf, stackframe, node, options) -- Taken from https://github.com/tjdevries/config.nvim/blob/85172d183c18968c6995ab7f9346a0551d47948c/lua/custom/plugins/dap.lua#L20-L33
        local name = string.lower(variable.name)
        local value = string.lower(variable.value)
        if name:match("secret") or name:match("api") or name:match("token") or value:match("secret") or value:match("api") then
          return "*****"
        end
...
local nvim_jdtld_lazy_spec = bpu:declare_lazy_spec(
  "config.infra.plugins.nvim-dap-go",
  {
    dependencies = {
      "dap",
    },
